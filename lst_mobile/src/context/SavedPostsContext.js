...
export function SavedPostsProvider({ children }) {
  ...
  const forgetDeletedPost=postId=>setSavedPostIds(ids=>ids.filter(id=>id!==String(postId)));
  const value=useMemo(()=>({savedPostIds,savedPostsLoading,isPostSaved:id=>savedPostIds.includes(String(id)),toggleSavedPost,forgetDeletedPost}),[savedPostIds,savedPostsLoading]);
