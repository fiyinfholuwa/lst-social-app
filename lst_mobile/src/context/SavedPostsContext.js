...
export function SavedPostsProvider({ children }) {
  ...
  useEffect(()=>{if(!user){setSavedPostIds([]);setLoading(false);return;} setLoading(true);apiService.getSavedPostIds().then(setSavedPostIds).catch(error=>{console.error('Unable to load saved posts:',error);setSavedPostIds([]);}).finally(()=>setLoading(false));},[user]);
