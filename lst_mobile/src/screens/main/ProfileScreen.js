    ...
    <View style={[styles.screen, { backgroundColor: theme.background }]}>
      ...
      <ScrollView contentContainerStyle={[styles.content, { paddingBottom: tabBarHeight + 24 }]} showsVerticalScrollIndicator={false}>
        <View style={[styles.hero, { backgroundColor: theme.card, borderColor: theme.border }]}>
          ...
          <TouchableOpacity
            style={[styles.avatarFrame, { backgroundColor: theme.card }]}
            onPress={() => profile.avatar && setImageVisible(true)}
            disabled={!profile.avatar}
            accessibilityRole={profile.avatar ? 'button' : undefined}
            accessibilityLabel={profile.avatar ? 'View your full profile photo' : 'Your profile avatar'}
          >
            <Avatar uri={profile.avatar} size={84} style={styles.avatar} accessibilityLabel={`${profile.name}'s profile avatar`} />
            {profile.avatar ? <View style={[styles.avatarViewBadge, { backgroundColor: theme.card }]}><Icon name="expand-outline" size={12} color={theme.primary} /></View> : null}
          </TouchableOpacity>
          ...
          {profile.emailVerified ? (
            ...
            <TouchableOpacity
              style={[styles.verificationCta, { backgroundColor: theme.accentSoft, borderColor: theme.accent }]}
              onPress={() => navigation.navigate('VerifyEmail')}
              accessibilityRole="button"
              accessibilityLabel="Verify email"
            >
              <View style={[styles.verificationCtaIcon, { backgroundColor: theme.card }]}><Icon name="mail-outline" size={19} color={theme.accent} /></View>
              <View style={styles.verificationCtaCopy}>
                <Text style={[styles.verificationCtaTitle, { color: theme.text }]}>Email verification required</Text>
                <Text style={[styles.verificationCtaText, { color: theme.secondaryText }]}>Verify now to post, share and join communities</Text>
              </View>
              <View style={[styles.verificationCtaAction, { backgroundColor: theme.accent }]}>
                <Text style={styles.verificationCtaActionText}>Verify now</Text>
      ...
      <Modal visible={imageVisible} transparent statusBarTranslucent animationType="fade" onRequestClose={() => setImageVisible(false)}>
        <Pressable style={styles.imageViewer} onPress={() => setImageVisible(false)} accessibilityLabel="Close profile photo">
          <TouchableOpacity style={styles.imageClose} onPress={() => setImageVisible(false)} accessibilityLabel="Close profile photo"><Icon name="times" size={22} color="#FFFFFF" /></TouchableOpacity>
          <Pressable style={styles.fullImageFrame} onPress={() => {}}>
            <Image source={{ uri: resolveMediaUri(profile.avatar) }} style={styles.fullImage} resizeMode="contain" accessibilityLabel={`${profile.name}'s full profile photo`} />
          </Pressable>
