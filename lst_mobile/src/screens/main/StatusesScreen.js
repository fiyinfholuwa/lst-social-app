...
export default function StatusesScreen({ navigation }) {
  ...
  const showViewers = currentStatus => {
    const viewers = currentStatus.viewers || [];
    Alert.alert(
      'Seen by',
      viewers.length ? viewers.map(viewerItem => viewerItem.name).join('\n') : 'No one has seen this status yet.',
    );
  };
  const confirmDelete = currentStatus => {
    Alert.alert('Delete status?', 'This status will be removed for everyone.', [
      { text: 'Cancel', style: 'cancel' },
      {
        ...
        onPress: async () => {
          ...
          try {
            ...
            Alert.alert('Status not deleted', error.message || 'Please try again.');
          } finally {
            setDeleting(false);
          }
        },
      ...
      <KeyboardAvoidingView style={styles.overlay} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
        <View style={[styles.sheet, { backgroundColor: theme.card }]}>
          ...
          <TextInput
            ...
            style={[styles.input, { color: theme.text, backgroundColor: theme.background, borderColor: theme.border, fontSize: statusFontSize, lineHeight: statusLineHeight, height: 120, maxHeight: 120 }]}
            editable={!saving}
          />
