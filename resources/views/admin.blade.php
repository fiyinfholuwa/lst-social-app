...
<script>
    ...
    function memberDetailMarkup(payload) {
        ...
        <section class="detail-section"><h3>Account management</h3><div class="member-management"><form class="management-card member-ajax-form" action="${escapeHtml(payload.actions.verification)}"><h4>Email verification</h4><p>${member.email_verified ? `Verified ${escapeHtml(member.email_verified_at)}.` : 'Manually confirm this member’s email address.'}</p><input type="hidden" name="_method" value="PATCH"><input type="hidden" name="verified" value="${member.email_verified ? 0 : 1}"><button class="mini-btn ${member.email_verified ? 'danger' : 'approve'}" type="submit">${member.email_verified ? 'Remove verification' : 'Verify member'}</button></form><form class="management-card member-ajax-form" action="${escapeHtml(payload.actions.role)}"><h4>Access role</h4><p>Change the permissions assigned to this account.</p><input type="hidden" name="_method" value="PATCH"><select name="role">${roles}</select><button class="mini-btn" type="submit">Update role</button></form>
