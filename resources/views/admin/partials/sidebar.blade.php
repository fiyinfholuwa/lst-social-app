    ...
    <nav class="nav">
        @foreach($manage as $key)
            <a data-admin-link class="{{ ($section ?? '') === $key ? 'active' : '' }}" href="{{ url('/admin/'.$key) }}">
                <svg class="icon" aria-hidden="true">@switch($key)@case('moderation')<path d="M12 3 4 6v5c0 5.1 3.4 8.7 8 10 4.6-1.3 8-4.9 8-10V6zM9 12l2 2 4-5"/>@break @case('analytics')<path d="M4 20V10M10 20V4M16 20v-7M22 20H2"/>@break @default<circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.7 1.7 0 0 0 .3 1.9l.1.1-2.8 2.8-.1-.1a1.7 1.7 0 0 0-1.9-.3 1.7 1.7 0 0 0-1 1.6v.2h-4V21a1.7 1.7 0 0 0-1-1.6 1.7 1.7 0 0 0-1.9.3l-.1.1L4.2 17l.1-.1a1.7 1.7 0 0 0 .3-1.9A1.7 1.7 0 0 0 3 14H2.8v-4H3a1.7 1.7 0 0 0 1.6-1 1.7 1.7 0 0 0-.3-1.9L4.2 7 7 4.2l.1.1a1.7 1.7 0 0 0 1.9.3A1.7 1.7 0 0 0 10 3V2.8h4V3a1.7 1.7 0 0 0 1 1.6 1.7 1.7 0 0 0 1.9-.3l.1-.1L19.8 7l-.1.1a1.7 1.7 0 0 0-.3 1.9 1.7 1.7 0 0 0 1.6 1h.2v4H21a1.7 1.7 0 0 0-1.6 1z"/>@endswitch</svg>
                <span>{{ ucfirst($key) }}</span>@if($key === 'moderation' && \App\Models\SupportRequest::where('status','open')->count())<span class="badge">{{ \App\Models\SupportRequest::where('status','open')->count() }}</span>@endif
            </a>
        @endforeach
    </nav>
